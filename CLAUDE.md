...
Scadium — non-custodial, provably-fair Solana casino modeled after solpump.io, now with **13 games** (`apps/api/src/games/*`): Crash, Coinflip, Blackjack, Jackpot, Lottery, Dice, Limbo, HiLo, Mines, Plinko, Tower, Wheel, Keno (live by default with the first four). **It defaults to a play-money balance** (`User.playBalanceLamports`, seeded at 10 SOL): every casino game debits/credits Postgres, not the chain. **Players can also deposit and withdraw SOL with a real wallet** (custody, ADR 0005 — live on devnet): a deposit is a verified transfer to a server hot wallet credited to that same balance, a withdrawal is a server-signed transfer back; the first deposit converts the account from play-money to deposited SOL (`User.fundedAt`). Real money is a **fail-closed boot toggle** — `main.ts` runs `assertRealMoneyReady` and refuses to start with `REAL_MONEY_ENABLED` unless licence + KYC + geoblocking are configured (see `apps/api/src/compliance/`). On-chain Phases A–F shipped the Anchor programs (`scadium_vault`/`scadium_swap`/`scadium_lottery`), a `$SCAD` rewards economy, a SCAD/SOL CPMM pool + buy-and-burn, and on-chain lottery draws; the IDL + program keypairs are now **committed under `target/`** and CI runs `anchor build` + localnet integration + an IDL-drift gate. The chain layer is still **off-chain-first** — balances live in Postgres and the programs aren't yet mainnet-live for real money — but it is no longer unbuilt. Making it real money is the roadmap.

**Read the docs first** — the authoritative planning/state lives in repo docs, not GitHub: `docs/production-readiness.md` (go/no-go verdict per launch mode), `docs/audit/phase-0-ground-truth.md` (the finding register), `docs/roadmap-vs-solpump.md` (Now/Next/Later), `BACKLOG.md` (tracked work, tiered), `CHANGELOG.md` (what shipped). `ANALYSIS.md` is the older gap analysis (phased roadmap G→M). Ignore stale references to `~/.claude/plans/*`.

**Working process (solo developer — NO GitHub PRs/issues).** The owner works solo and does **not** want GitHub PRs, issues, or milestones. The process is: keep **`BACKLOG.md` + `CHANGELOG.md`** (and this CLAUDE.md) up to date, do the work, and **commit/push directly to `main`** — no PR, no issue tracking, no squash-merge ceremony. The `.claude/` pipeline commands (`/plan`, `/scadium-next`, `/dev-task`) and their GitHub-issue flow are **deprecated for this repo** — don't create issues or open PRs unless the owner explicitly asks. Because there is no PR/CI review gate before merge, **you are the gate**: before pushing to `main`, run the full local CI-equivalent suite (build/typecheck/lint/test + integration + a fresh `prisma migrate deploy`) and secret-scan the diff; money/fairness/security regressions are blocking and **nothing lands on `main` red or without its mandated tests**. CI still runs on push to `main` (build/integration/anchor/secret-scan/trivy/helm/e2e) — treat a red post-push CI as an immediate fix. **Always verify an audit/finding premise against the real code before implementing** (the audit has been wrong — e.g. #3 was already guarded at the service layer).
...
- `infra/docker-compose.yml` — Postgres 16 + Redis 7 for local dev, plus `solana` (a local `solana-test-validator`) that the custody chain suite needs. Redis is now **core**: BullMQ queues (worker), distributed locks, and the Socket.io Redis adapter for multi-replica broadcast. `infra/helm/scadium` is the production Helm chart (CI lints + renders it).
...
**Coinflip is request-driven and transactional.** `apps/api/src/games/coinflip/coinflip.service.ts` runs each create/join/house/cancel inside `prisma.$transaction` and broadcasts only after commit. A fresh `Seed` (serverSeed) is committed per flip at create; the result is keyed by the player seed + monotonic nonce of whoever did NOT pick the flip's timing — the joiner in PvP, the creator when they send it to the house (`vsHouse`; no joiner, `winnerId` null when the house wins). The winner is credited 1.9× the stake (5% edge). One `Bet` row per human side. Unjoined PvP flips expire (`COINFLIP.OPEN_TTL_MS`) through the shared `cancel-open-flip.ts` CAS + refund, which cancel, the expiry sweep and RG blocks all use.

**Round loops (crash/jackpot/lottery) share `apps/api/src/games/round-loop.ts`:** settles run with an explicit tx budget and are retried with backoff (then a fallback, never a freeze); `RoundTimers` + a generation make stale timers and abandoned async steps no-ops; a watchdog resumes a stalled loop; boot recovery settles only what is DUE and resumes the live round/draw. Crash drains its in-flight round on SIGTERM. Crash, jackpot and lottery results fold in a public drand beacon round published after betting closes (ADR 0004, `apps/api/src/beacon/`). Broadcasts carry a display handle + opaque `publicPlayerId` (`common/public-player.ts`), never userId/wallet; `/me` returns the caller's `publicId`.
...
**Custody (ADR 0005, `apps/api/src/custody/`).** SOL deposits/withdrawals against ONE hot wallet (the treasury, `CUSTODY_HOT_WALLET_SECRET_KEY`); the Postgres ledger stays the source of truth and `applyBalanceDelta` the only mutation point. `custody-chain.ts` is the only Solana I/O (a seam: web3.js in prod, an in-memory ledger in `test/custody-harness.ts`). Deposits are verified by the treasury's balance delta, attributed to the account owning the SENDING wallet, credited once (`CustodyTransfer.txSignature` unique) or held with a reason and retried by the `custody` job's history scan. Withdrawals store their signature + blockhash expiry BEFORE broadcast and re-sign/refund only once that signature is provably dead — never on a thrown send. `CustodyRuntime` activates only when the RPC's genesis hash proves `SOLANA_NETWORK`, never on mainnet. **Economy rules** (`custody/economy.ts`, on while `CUSTODY_ENABLED`): one balance column holds play-money AND deposited SOL, so nothing may move value from play to real — jackpot/lottery (and hidden blackjack) take deposited SOL only, a PvP coinflip joins only its creator's balance type, airdrop + race are play-only, referral commission needs both sides in the same economy, a first deposit waits for open play positions. Any NEW path that moves value between players, or credits something earned with play-money, must go through these rules. The emergency stop is the global pause, not `CUSTODY_ENABLED=false`.
