...
- [x] **Hard daily cap on container active time.** `DAILY_ACTIVE_SECONDS` (default 3600) is accounted in the container Durable Object's storage; past it the API returns 503 and the live games are offline until midnight UTC. Combined with the cron stopping the container after its sweep, an unvisited site costs **$0** and the worst case is bounded rather than merely unlikely. Raise it when the project goes always-on — which also needs Neon off the free plan.
