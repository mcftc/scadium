...
- [x] **Hard daily cap on container active time.** `DAILY_ACTIVE_SECONDS` (default 3600) is accounted in the container Durable Object's storage; past it the API returns 503 and the live games are offline until midnight UTC. **Corrected 2026-09-23:** the first version charged HTTP request gaps, and an open WebSocket keeps the container awake with no requests, so the cap did not bound the worst case. It now charges *running* time from a heartbeat and stops the container (gracefully — crash drains its round) once the cap is spent. Raise it when the project goes always-on — which also needs Neon off the free plan.
