...
- [ ] **C3 · Seed rotation leaks the active seed mid-round → deterministic mines/tower/hilo wins.** `rotateServerSeed` must refuse (or defer) while any stateful round for that user/seed is active. `apps/api/src/fairness/seed-manager.service.ts:95`, `games/instant/stateful-round.ts:189`.
- [ ] **H2 · Coinflip `cancel()` TOCTOU duplicates money.** Non-locking status read + unconditional `update` by id. Make the final flip a guarded CAS (`updateMany where status='open'`) and/or wrap in `withSerializable`. `apps/api/src/games/coinflip/coinflip.service.ts:465`.
- [ ] **H3 · Crash auto-cashout can pay ZERO below the bust point.** Pay the committed target when `target < bust` regardless of tick timing; fix the `BigInt(Math.floor(m*100))` 0.01× underpay. `apps/api/src/games/crash/crash.engine.ts:374`.
- [ ] **H1 · Blackjack restart during betting confiscates stakes.** Debit commits before `deal()` persists `stateJson`; boot recovery sees empty `{}` and refunds nothing. Persist accepted bets at bet time (or refund on recovery). `blackjack.engine.ts:755`.
- [ ] **H11 · Hourly settle jobs run mid-hour on partial data.** Worker fires every 5 min but jobs target the *current* hour → airdrop/dividends/mining/vault settle on ~5 min of data and airdrop rejects tips for ~55 min. Settle only *elapsed* hours (or true top-of-hour Cron) + backfill missed hours. `airdrop.engine.ts:145`, `distribution.service.ts:44`, `block-mining.service.ts:56`, `apps/worker/src/main.ts:156`.
- [ ] **H20 · RG limits bypassed on coinflip/lottery.** Both pass `0n` to `rg.assertCanWager`. Pass the real stake. `coinflip.service.ts:151`.
- [ ] **H18 · Affiliate commission has no payout path.** Accrues (crash/coinflip only) but is unclaimable; dashboard shows uncollectable SOL. Build the claim path or remove the claim UI + wire the other games.
