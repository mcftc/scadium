...
/**
 * Procedurally place `starCount` stars on a spherical shell. Intentionally
 * random (this is decorative geometry) — kept as a module-level helper so the
 * `Math.random()` calls run outside render (invoked once via a lazy state
 * initializer), not as an impure render-time computation.
 */
function buildStars(
  starCount: number,
  radius: number,
...
export function Starfield({
  ...
  // Built once per mount (stars don't need to regenerate on prop changes, and
  // doing so would re-shuffle the whole sky). Lazy initializer keeps the random
  // generation out of render.
  const [[positions, colors]] = useState(() => buildStars(starCount, radius, depth));
