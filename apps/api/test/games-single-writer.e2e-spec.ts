...
import { prisma, gw, offChain, pow, affiliatesStub } from './engine-harness';
...
describe('game engines single-writer leader election (issue #86)', () => {
  ...
  it('blackjack: only the leader creates the Main Table', async () => {
    ...
    const since = new Date();
    const a = new BlackjackEngine(prisma as never, gw(), offChain, pow(), affiliatesStub(), redis(r1));
    const b = new BlackjackEngine(prisma as never, gw(), offChain, pow(), affiliatesStub(), redis(r2));
    destroyers.push(() => a.onModuleDestroy(), () => b.onModuleDestroy());
    await Promise.all([a.onModuleInit(), b.onModuleInit()]);
    await waitForLeader(a, b);
    for (let i = 0; i < 40; i++) {
